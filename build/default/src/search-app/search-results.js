...
class ResultsApp extends PolymerElement {
  ...
  handleError(e) {
    ...
    let iron = dom(e.target); // Grabs the error container

    let errorContainer = iron.parentNode.parentNode.children[3]; // Gets the label element that will hold the error

    let labelElement = errorContainer.children[0]; // console.log(labelElement);
    // Provides the label element with the error text

    labelElement.innerHTML = this.error.error; // Displays the error container

    errorContainer.style.display = 'flex'; // Clears the error container and it's content

    setTimeout(function () {
      // Clears the error message in the label element
      labelElement.innerHTML = ''; // Makes the error container disappear
  ...
  handleClick(e) {
    ...
    let span = dom(e.target); // closes the error container
